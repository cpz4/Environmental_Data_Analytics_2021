...
# Matrices ----
matrix1 <- matrix(1:20, nrow = 5, ncol = 4) # Default is to fill the matrix by column. 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) # Filling the matrix by row. 
matrix2
...
# Default: [row, column]
# Access elements inside a matrix 
matrix1[4, ] # Get row 4
matrix1[ , 3] # Get column 3
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
#celcius_highs <- funcion(x) return1 <-(x-32)*(5/9))
#return(list(return1 - return1)
...
today <- Sys.Date()
format(today, format = "%B") # non-abbreviated month
#format(today, format = "%A")
#format(today, format = "%b") # abbreviated month
#format(today, format = "")
...
#there are other similar functions ydm(), mdy(), my(), etc
```
