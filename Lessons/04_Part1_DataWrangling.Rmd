...
# matrix filtering                      # filtering only the rows that have 0 on column depth. 
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering
# subset                  filter(dataset, column == characteristic) 
...
# take Paul Lake OR Peter Lake
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")

# Filtering the lakes we're not interested into to get Peter or Paul. 
# When you just want to remove one of the lakes from the dataset. 
...
# %in% useful to select based on more than 2 elements. 
...
```{r creating a subset from a dataset}
#                             (dataset, column, range of columns)
...
```{r}
# dataset you want to add things to <- mutate(existing dataset, new column = whatever the new colums will be equal to)
...
#                                                         columns I want first, month, rest of the columns. 
...
# Take first row [1] from column from dataset, then take last row [21613]
# dataset$column[row #]
...
# new dataset <- old %in% modifications. Only conditions inside since we already mentioned our original dataset at the beggining. 

NTL.phys.data.processed <- 
  NTL.phys.data %>% #original data "then"
