...
# Some packages are umbrellas under which other packages are loaded, meaning that by installing one you're installing multiple packages. 
...
> Answer: It shows the packages being installed, including all the packages installed with the umbrella.
...
# Rename columns
# Create a vector with the name of each column using function c: c is short for column bind. 
...
# use matrix notation to access specific parts. rows 30000 to 30005 from columns 3, 8, 14. 
...
```{r removeNA}
# Create a new object to store the data without NAs. 
# This removes all rows that have NAs. 
...
#datasetName$columnWithDates <- as.Date(datasetName$columnWithDates, format = specify)
...
```{r fixformartDate}
# Select the column datetime from dataset USGS.flow.data. 
# Use the function format to establish the format of the date: %y%m%d
#This changes the type from date to character. 
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d") 

# Error is: All years previous to 1969, R assigned the first two digits to 20 instead of 19. So we have years like 2068. 
# paste0 glues strings.  
# For all data greater than 191226, paste a 19 instead of a 20.  
create.early.dates <- (function(d) { paste0(ifelse(d > 191226,"19","20"),d)})
# Here we add the 19 to those initial observations. "19271001". 
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

# Now that the data has been fixed, we change from chr type to data. 
USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%Y%m%d") 

```

## Saving datasets. Store processed data in your local machine. 
