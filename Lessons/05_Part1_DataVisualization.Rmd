...
# Scatterplot
# figure(dataset, easthetics(x = , y =))
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point() # how do you want the variable to be plotted
...
O3plot2 <- ggplot(EPAair) +
  #geom_point(aes(x = Date, y = Ozone, color = "blue")) #wrong, it understands that you're adding another leyend to the plot, not specifying the color. 
  geom_point(aes(x = Date, y = Ozone), color = "blue") # closed parenthesis and specify color outside
print(O3plot2)

# Add additional variables
# How could you automatically assign a marker color to a variable? 
# Assign a different shape based on the year. Circle to 2018, triangle to 2019...
# Color. All the info from the same site will have the same color. 
PMplot <- 
...
PMplot.faceted <-
  ...
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) # one plot for each site, 
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() # reverses the y axis, 0 goes on top. 
print(tempvsdepth)
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Color is about the countour, fill is the filling. 
print(Nutrientplot3)
...
print(Nutrientplot4)
# binaxis means that we're stacking the obervations over Y. 
# dodge: so data points don't overlay each other but are spead out going to the sides. 
# You can see that the concentration of tnug is more spread across concentration values, 

# Violin plot
# Show you a density plot, the distribution of the data
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
