...
# BAR CHART. Useful to visualize categorical data. 

# ggplot(dataframeName, aestherics(x = attribute from the data we want to plot)) + 
# geom_bar()
ggplot(USGS.flow.data, aes(x = discharge.mean.approval)) +
  geom_bar()

# We could also omit NA's and plot the same info without NAs. 
...
```{r, fig.height = 3, fig.width = 4}
# HISTOGRAM. Usefulto visualize numerical data. 

ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean))

# binwidth = 10 is telling R to group of data in groups of 10. 
ggplot(USGS.flow.data) +
...
# Limit the data shown in the histogram to see clearer tendencies. 
...
# Useful to compare different aspects of the data, in this case the min, max, and mean values of gage height. 
# lty is line type
ggplot(USGS.flow.data) +
  geom_freqpoly(aes(x = gage.height.mean), bins = 50) +
  geom_freqpoly(aes(x = gage.height.min), bins = 50, color = "darkgray") +
  geom_freqpoly(aes(x = gage.height.max), bins = 50,  lty = 100) +
...
```{r, fig.height = 3, fig.width = 4}
# BOX PLOT. Useful to visualize summary stats, outliers. 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

# Groups of data. Variance across different slices of data. 
ggplot(USGS.flow.data) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

# Width shows how many data points are distributed within the lines. 
ggplot(USGS.flow.data) +
