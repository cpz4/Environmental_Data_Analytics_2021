...
# facet wrap (specify each variable you want to wrap in this case lake). 
# facet grid (more than one level in your category, for two discreate variables).  


peter.plot.1 <- 
  ggplot(subset(PeterPaul.chem.nutrients, depth == 0), aes(x =      daynum, y = temperature_C)) +
  geom_point(color = year4) +
  labs (color = "Year", y = "Temperature (C)") +
  facet_wrap(vars(lakename), nrow = 2 ) +
  ylab("Surface Teperature") +
...
peter.plot.2 <- ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth)) +
  geom_point( size = 0.5 ) +
  labs (color = "Depth", y = "Temperature (C)") +
...
plot.3 <- ggplot(EPAair, aes(x = PM2.5, y = Ozone, color = SITE_LATITUDE, )) +
  geom_point( alpha = 0.5 ) +
  geom_smooth(method = lm, color = "black") + 
  labs(color = "Site Latitude", x = "Mean AQI PM2.5" , y = "Mean AQI Ozone")
print(plot.3)
...
plot.4 <- ggplot(EPAair, aes(x = Year, y = PM2.5 )) +
  geom_point( alpha = 1 ) 
print(plot.4)


plot.5 <- ggplot(EPAair, aes(x = Year, y = PM2.5 )) +
  geom_boxplot( alpha = 1 ) 
print(plot.5)

plot.5 <- ggplot(EPAair, aes(x = Year, y = PM2.5 )) +
  ggeom_violin( alpha = 1 ) 
print(plot.5)
